...
missing_files = [path for path in (MODEL_PATH, SCALER_PATH, COLUMNS_PATH) if not path.exists()]
if missing_files:
    st.error(
        "Missing model files: " + ", ".join(str(path.name) for path in missing_files)
        + ". Please add them under the models/ directory."
    )
    st.stop()
